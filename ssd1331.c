...
uint8_t SPIWrite(uint8_t *buffer, int bufferLength) {
    int i;
...
void SSD1331_begin()
{
	...
	_delay_ms(10);
	OLED_RST_PORT &= ~(1 << OLED_RST_PIN); //reset low
	_delay_ms(10);
	OLED_RST_PORT |= 1 << OLED_RST_PIN;  //reset high
...
void SSD1331_Rotate(int Rotate)
{
	...
	{
		command(SET_REMAP);            	//set remap & data format
		command(0x60);   				//normal 0x72   180 0x60 
	}
	else
	{
		command(SET_REMAP);            	//set remap & data format
		command(0x72);   				//normal 0x72   180 0x60 
	}
}
...
void SSD1331_num(unsigned char x, unsigned char y, double num,unsigned char decimal_point, unsigned char Size, unsigned char Mode, unsigned short hwColor)
{
  char str[100]; //safe to assume
  //dtostrf(num,0, decimal_point,str);
  itoa(num, str, (int) decimal_point);
  //printf("%s",str);
  SSD1331_string(x,y,str,Size,Mode,hwColor);
}
